...
class ProductController extends \BaseController
{
    ...
    public function onConstruct()
    {
        ...
        // Stripe
        \Stripe\Stripe::setApiKey($this->api->stripe->secretKey);
    ...
    public function viewAction($slug)
    {
        ...
        $discount = null;
        $discount_price = null;
        $promo_code = $this->request->get('promo');
        if ($promo_code) {
            $promo = new \ProductPromo();
            $percent_off = $promo->check($promo_code, $product->id);
        ...
        // Facebook Login
        // ---------------------------
        $after_fb = '/product/view/' . $slug;
        $helper = $this->facebook->getRedirectLoginHelper();
        $fbLoginUrl = $helper->getLoginUrl($this->api->fb->redirectUri, (array) $this->api->fb->scope);
    ...
    private function _createPurchase(\Product $product, $gateway = false, $transaction_id = false)
    {
        ...
        if ($promo_applied) {
            $userPurchase->promo_code = $this->promo_code;
        }
        $userPurchase->save();
